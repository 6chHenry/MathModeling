...
class ModelRobustnessAnalyzer:
    ...
    def _visualize_ablation_results(self, results_df):
        ...
        axes[1].set_xlabel('R^2变化')
        axes[1].set_title('特征消融对R^2的影响')
        axes[1].grid(True, alpha=0.3)
    ...
    def feature_perturbation_analysis(self, perturbation_levels=[0.8, 0.9, 1.1, 1.2]):
        ...
        # 保存结果
        results_df.to_csv('feature_perturbation_results.csv', index=False)
    ...
    def _visualize_perturbation_results(self, results_df):
        ...
        plt.ylabel('特征')
        plt.tight_layout()
        plt.savefig('feature_perturbation_heatmap.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def generate_comprehensive_report(self):
        ...
        if self.df is None or self.X is None or self.model is None:
            print("错误：数据或模型未准备好")
            return None
        ...
        # 保存报告
        import json
        with open('robustness_analysis_report.json', 'w', encoding='utf-8') as f:
