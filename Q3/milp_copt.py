...
class CinemaSchedulingOptimizer:
    def __init__(self, cinema_file, movies_file):
        ...
        # 题材播放次数限制 - 修正：Horror没有下限
        self.genre_limits = {
            'Animation': {'min': 1, 'max': 5},
            'Horror': {'max': 3},  # 修正：Horror只有上限，没有下限
            'Action': {'min': 2, 'max': 6},
            'Drama': {'min': 1, 'max': 6}
        }
    ...
    def _optimize_with_copt(self):
        ...
        # 移除原有的每部电影放映次数限制约束（题目没有要求）
        # 原约束4已删除

        # 约束4（原约束5）: 设备连续运行时长限制（每连续9小时内累计播放不超过7小时）
        print("添加设备运行时长约束...")
        for room in x:
            # 遍历所有可能的9小时滑动窗口
...
def main():
    # 创建优化器实例
    optimizer = CinemaSchedulingOptimizer('D:\PythonProjects\MCM\input_data\df_cinema.csv',
    ...
    if status == 1:
        ...
        result_df.to_csv('D:\PythonProjects\MCM\output_result\df_result_2_copt.csv', index=False)

        print(f"排片计划已保存到 df_result_2_copt.csv")
        ...
        # 3. 时间格式检查
        print(f"\n时间格式检查:")
        print("所有时间均为标准24小时制格式（跨日时间显示为00:xx-03:xx）")
